...
function TradeSkillDW_SkillTab_OnPreClick(self)
	--[[if CheckIsValueInTable(knownSkills[SKILL_SECONDARY], self.skill) then
		return
	end

	for _, v in pairs(tabPool[SKILL_PRIMARY]) do
		if (v ~= self) then
			v:SetChecked(nil)
		end
	end]]
...
function TradeSkillDW_SlashHandler(msg, editbox)
	...
	if command == "" then
		print("TradeSkillDW slash commands:\n/tsdw strata LEVEL\ncureent strata is "..TradeSkillDW_Settings.frameStrata)
